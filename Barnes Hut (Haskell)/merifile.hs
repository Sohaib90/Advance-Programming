...
updateBody foldme (Body (posx,posy) vel mass clr) = let x = (removeme (foldr(\x y -> x:y) [] foldme) (Body (posx,posy) vel mass clr)) in 
                               Body (distance (velocity (getacc (resultant([getforce b (Body (posx,posy) vel mass clr)| b<-x])) (Body (posx,posy) vel mass clr)) (Body (posx,posy) vel mass clr)) (Body (posx,posy) vel mass clr)) vel mass clr
