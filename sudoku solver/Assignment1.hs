...
sudoku :: Board -> Board
--sudoku b | b == inputBoard = solvedBoard
sudoku b  =  fromElements (head (sudokuElements  [fst (toElements b)] (snd (toElements b))))
...
sudokuElements bes (x:xs) = sudokuElements func xs 
                            where func = [(x,valid):board | board<-bes, valid<-validVals board x]               
...
--    | bes \\ solvedBoardElements == [] = [1..9]    | otherwise = []\
validVals [] _ = []
validVals blist a = [1..9] \\ ((map (\x-> snd x) (filter (\x -> fst (fst x) == fst a ) blist))
                             ++ (map (\x-> snd x) (filter (\x -> snd(fst x)== snd a ) blist))
                            ++ (map (\x-> snd x) (filter (\x -> ((snd(fst x))`div` 3== (snd a)`div` 3) && ((fst(fst x)) `div` 3 == (fst a) `div` 3)) blist)))
...
--toElements b 
--    | b==inputBoard = (inputBoardElements, inputBoardEmpty) | otherwise = error "toElements not implemented"
toElements [] = ([],[])
toElements bes = ((filter (\x-> snd x /= 0) (coordinate bes)),(map (\x->fst x) (filter (\x-> snd x ==0) (coordinate bes))))
...
--fromElements bes 
--    | sort bes == solvedBoardElements = solvedBoard | otherwise = error "fromElements not implemented"
fromElements bes = let fn = (groupBy (\x y -> (fst (fst x) >= fst (fst y))) (sort bes)) in [map (\ (x,y) -> y) x | x <- fn] 


coordinate:: Board -> [BoardElement]
coordinate [] = []
coordinate (a:as) =  (zip (map (\x-> (((abs((length (a:as))-9)),x))) [0..8]) a ) ++ coordinate as
